...
//e-fun begin change
if (!root.hyperlinks) root.hyperlinks = [];
if (childNode.tagName === "A") {
    var tg = childNode.getAttribute('target');
    var hr = childNode.href;
    if (hr.baseVal) hr = hr.baseVal;

    if (!hr)
        hr = childNode.getAttributeNS('http://www.w3.org/1999/xlink', 'href');

    if (hr) {
        for (var i1 = 0; i1 < container.elements.length; i1++) {
            var eb = container.elements[i1].bounds
            root.hyperlinks.push (
                {
                    "type": "rect",
                    "target": tg,
                    "href": hr,
        ...
        for (var i1 = 0; i1 < container.textNodes.length; i1++) {
            for (var i2 = 0; i2 < container.textNodes[i1].textBounds.length; i2++) {
                var tb = container.textNodes[i1].textBounds[i2].bounds
                root.hyperlinks.push (
                    {
                        "type": "rect",
                        "target": tg,
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 5:
                    ...
                    //Logger.destroy(instanceName);  // change e-fun
                    //CacheStorage.destroy(instanceName);  // change e-fun
                    return [2 /*return*/, {canvas: canvas, hyperlinks: root.hyperlinks}]; // change e-fun
            }
        });
    }); };
